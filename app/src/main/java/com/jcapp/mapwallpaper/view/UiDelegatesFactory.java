...
public class UiDelegatesFactory {
    ...
    public void onBindViewHolder(SkuRowData data, RowViewHolder holder) {
        Objects.requireNonNull(uiDelegates.get(data.getSku())).onBindViewHolder(data, holder);
    }

    public void onButtonClicked(SkuRowData data) {
        Objects.requireNonNull(uiDelegates.get(data.getSku())).onButtonClicked(data);
    }
}
